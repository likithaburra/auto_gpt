        ...
        while self.attributes.state != AgentState::Finished {
            match &self.attributes.state {
                ...
                AgentState::UnitTesting => {
                    ...
                    // Build and Test Code
                     PrintCommand::UnitTest.print_agent_message(self.attributes.position.as_str(), "Backend Code unit Testing: building project...");
                     // Build Code
                     let build_backend_server:std::process::Output=Command::new("cargo").arg("build").current_dir(WEB_SERVER_PROJECT_PATH).stdout(Stdio::piped()).stderr(Stdio::piped()).output().expect("Failed to build backend application");
                    //  Determine if build errors
                    if build_backend_server.status.success(){
                        self.bug_count=0;
                        PrintCommand::UnitTest.print_agent_message(self.attributes.position.as_str(), "Backend Code Unit Testing: Test server build successful...");
                        ...
                        if self.bug_count>2{
                            PrintCommand::Issue.print_agent_message(self.attributes.position.as_str(), "Backend Code Unit Testing: Too many bugs found in code");
                ...
                //   Extract API Endpoints
                    let api_endpoints_str:String=self.call_extract_rest_api_endpoint().await;
                    // Convert API Endpoints into Values
                    let api_endpoints:Vec<RouteObject>=serde_json::from_str(api_endpoints_str.as_str()).expect("Failed to decode API Endpoints");
                ...
                PrintCommand::UnitTest.print_agent_message(self.attributes.position.as_str(), "Backend Code Unit Testing: Start web server");
                ...
                PrintCommand::UnitTest.print_agent_message(self.attributes.position.as_str(), "Backend Code Unit Testing: Launching tests on server in 5 seconds");
                ...
                for endpoint in check_endpoints{
                    ...
                    match check_status_code(&client, &url).await {
                        ...
                        Err(e)=>{
                            run_backend_server.kill().expect("Failed to kill backend web server");
                ...
                PrintCommand::UnitTest.print_agent_message(self.attributes.position.as_str(), "Backend testing completed...");
                run_backend_server.kill().expect("Failed to kill backend web server on completion");
...
mod tests {
    ...
    async fn tests_writing_backend_code() {
        ...
        let factsheet_str = r#"
{
    "project_description": "Build a full stack website with user login and logout that shows latest Forex prices",
    "project_scope": 
        {
